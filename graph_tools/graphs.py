...
#----------------------------------------------------------------------------------------------------
		...
		self.legendAbsValues = 0	              # view abs. values in legend: 1 = yes, 0 = no
...
#----------------------------------------------------------------------------------------------------
	...
	def build_legend(self, barColors, barValues):
		"""return the legend"""
		...
		for color in barColors:
			...
			if self.legendAbsValues:
				sum = 0
				for charts in barValues:
					for labels in charts: sum += labels[i]
				legend += '<td style="' + self.__cssLEGEND + '" nowrap>(' + self.absValuesPrefix + str(sum) + self.absValuesSuffix + ')</td>'
	...
	def create(self):
		...
		for i in range(labels):
			...
			for v in drv:
				...
				if self.type == 'hbar' or self.type == 'vbar' or j == 0:
					if v > max: max = v
		...
		if self.barLevelColors:
			for i in range(0, len(self.barLevelColors), 2):
				if self.barLevelColors[i] == 'MAX': self.barLevelColors[i] = max
		...
		if self.legend and self.type != 'pbar' and self.type != 'fader':
			...
			graph += self.build_legend(bc, val)
			graph += '</td></tr></table>'
